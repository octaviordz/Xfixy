...
module Logging =
    ...
    let setLogFunc (logFunc: LogLevel -> Func<string> -> exn -> obj array -> unit) =
        logf <- logFunc
        ()
    ...
    [<Sealed>]
    type Logger() =
        member self.LogTrace(msg: string) = log (TraceLevel msg)
        member self.LogTrace(msg: string, [<ParamArray>] args: obj array) = log (TraceLevel(msg, args))
        member self.LogTrace(ex: exn, msg: string) = log (TraceLevel(ex, msg))
        member self.LogTrace(ex: exn, msg: string, [<ParamArray>] args: obj array) = log (TraceLevel(ex, msg, args))
        member self.LogTrace(msgFunc: Func<string>) = log (TraceLevel(msgFunc))
        member self.LogDebug(msg: string) = log (DebugLevel msg)
        member self.LogDebug(ex: exn, msg: string) = log (DebugLevel(ex, msg))
        member self.LogDebug(msgFunc: Func<string>) = log (DebugLevel(msgFunc))
        member self.LogInformation(msg: string) = log (InfoLevel msg)
        member self.LogInformation(msg: string, [<ParamArray>] args: obj array) = log (InfoLevel(msg, args))
        member self.LogInformation(ex: exn, msg: string) = log (InfoLevel(ex, msg))
        member self.LogInformation(ex: exn, msg: string, [<ParamArray>] args: obj array) = log (InfoLevel(ex, msg, args))
        member self.LogInformation(msgFunc: Func<string>) = log (InfoLevel(msgFunc))
        member self.LogWarning(msg: string) = log (WarnLevel msg)
        member self.LogWarning(ex: exn, msg: string) = log (WarnLevel(ex, msg))
        member self.LogWarning(ex: exn, msg: string, [<ParamArray>] args: obj array) = log (WarnLevel(ex, msg, args))
        member self.LogWarning(msgFunc: Func<string>) = log (WarnLevel(msgFunc))
        member self.LogError(msg: string) = log (ErrorLevel msg)
        member self.LogError(ex: exn, msg: string) = log (ErrorLevel(ex, msg))
        member self.LogError(ex: exn, msg: string, [<ParamArray>] args: obj array) = log (ErrorLevel(ex, msg, args))
        member self.LogError(msgFunc: Func<string>) = log (ErrorLevel(msgFunc))
        member self.LogFatal(msg: string) = log (FatalLevel msg)
        member self.LogFatal(ex: exn, msg: string) = log (FatalLevel(ex, msg))
        member self.LogFatal(msgFunc: Func<string>) = log (FatalLevel(msgFunc))
